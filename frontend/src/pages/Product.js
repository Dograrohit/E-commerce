                  ...
                  <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
                      {
                        ProductData.images.map((item,index)=>(
                          <img onClick={()=>changeImage(item)} src={item} key={index} className='w-[24%] sm:w-full sm:mb-3 flex-shrink-0 cursor-pointer border rounded-[5%] border-orange-300'></img>
                        ))
                      }
                  </div>
                  <div className='w-full sm:w-[80%]'>
                       <img className={`w-full h-auto border border-orange-300 rounded-[5%] transition-opacity duration-500 ease-in ${fade ? "opacity-100" : "opacity-0"}`} src={image}></img>
                  </div>
              </div>
